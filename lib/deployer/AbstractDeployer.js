...
export class AbstractDeployer {
  ...
  async checkDirectories(conn, projectFolder, stepNumber, connectionMessage) {
    ...
    await AbstractDeployer._runDeploymentStep(
      conn,
      command,
      task,
      stepNumber,
  ...
  async setCurrentRevision(conn, releaseDir, stepNumber, connectionMessage) {
    ...
    // Use AbstractDeployer._runDeploymentStep to execute the command and log messages
    await AbstractDeployer._runDeploymentStep(
      conn,
  ...
  //     const { stdout, stderr } = await asyncWrapper(conn, command);

  //     if (stderr) {
  //       actionLogger.error(stderr || "Unknown error", chalk.red); // Ensure stderr is always a string
  //     }
  //     if (stdout) {
  //       actionLogger.info(stdout || "No output", chalk.green); // Ensure stdout is always a string
  //     }
  ...
  //     );
  //   } catch (error) {
  //     console.error("Error during deployment:", error); // Log the error
  //     throw new DeploymentError(
