...
export class AbstractDeployer {
  ...
  async checkDirectories(conn, projectFolder, stepNumber, connectionMessage) {
    ...
    await this._runDeploymentStep(conn, command, stepNumber, connectionMessage);
  }
  ...
  async _runDeploymentStep(conn, command, stepNumber, message) {
    try {
      actionLogger.info(
