...
export class GitDeployer extends AbstractDeployer {
  ...
  async deploy() {
    ...
    try {
      ...
      // deploy:migrate
      try {
        await this.migrate(this.conn, releaseDir, 1, connectionMessage);
      } catch (error) {
      ...
      // deploy:migrating
      try {
        await this.migrating(this.conn, releaseDir, 1, connectionMessage);
      } catch (error) {
        actionLogger.error(
      ...
      // deploy:log_revision
      try {
        await this.logRevision(this.conn, releaseDir, 1, connectionMessage);
      } catch (error) {
        actionLogger.error(
  ...
  async cloneRepo(conn, branch, repoUrl, stepNumber, connectionMessage) {
    ...
    await asyncWrapper(conn, cloneCommand);
